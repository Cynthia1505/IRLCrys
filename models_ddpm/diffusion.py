...
class CSPDiffusion(nn.Module):
    def __init__(self,timesteps, run_type) -> None:
        super(CSPDiffusion, self).__init__()
        self.decoder =CSPNet(hidden_dim = 512,max_atoms = 100,
                             num_layers = 6, act_fn = 'silu', dis_emb = 'sin',
                              num_freqs = 128, edge_style = 'fc',max_neighbors= 20,
                             cutoff = 7, run_type = run_type, ln = True, ip = True, smooth = False,pred_type = False)
    ...
    @torch.no_grad()
    def sample_step(self, batch, atom_types, frac_coords, lattices, num_atoms, t,
                     step_lr=1e-5):
        ...
        pred_l, pred_x = self.decoder(
            time_emb, atom_types, x_t, l_t, num_atoms, batch.batch
        )
        pred_x = pred_x * torch.sqrt(sigma_norm)
        ...
        pred_l, pred_x = self.decoder(
            time_emb, atom_types, x_t_minus_05, l_t_minus_05, num_atoms, batch.batch
        )
        pred_x = pred_x * torch.sqrt(sigma_norm)
        ...
        frac_coords_next = x_t_minus_1 % 1.
        lattices_next = l_t_minus_1

        return frac_coords_next, lattices_next
